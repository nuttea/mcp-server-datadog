...
echo "Setting up Datadog MCP Server for Gemini CLI..."
...
if [ ! -f "$CONFIG_FILE" ]; then
  echo "Creating new Gemini CLI config..."
  cat > "$CONFIG_FILE" << EOF
{
  "mcpServers": {}
}
...
echo "Adding Datadog MCP server to Gemini CLI config..."
...
if command -v jq &> /dev/null; then
  ...
  echo "✅ Datadog MCP Server configured for Gemini CLI!"
else
...
echo "2. Test: Ask Gemini 'List my Datadog notebooks'"
echo ""
